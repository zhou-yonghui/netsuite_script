 ...
 * @NModuleScope public
 */
define(['N/search', 'N/url', 'N/record', './handlebars.min'], function (search, url, record, Handlebars) {
    // 显示进度的样式
    ...
    */
    function getFolderId(path) {
        try {
            ...
            if (!paths || paths.length < 1 || !paths[0] || !paths[paths.length - 1])
                return null;
            var filters = [];
            ...
            folderSearch.run().each(function (result) {
                ...
                var name = result.getValue(result.columns[2]);
                var parent = result.getValue(result.columns[1]);
                var row = new Object();
                row["name"] = name;
                row["parent"] = parent;
                nodes[id] = row;
            ...
            for (var i = 1; i < paths.length; i++) {
                ...
                for (key in nodes) {
                    var node = nodes[key];
                    if (node["name"] == paths[i] && node["parent"] == nodePath) {
                        nodePath = key;
                        findFlag = true;
                        break;
                    }
    ...
    //客户端脚本显示Map/Reduce脚本的运行状态
    function showReportProcess() {
        var taskId = document.getElementById('custpage_rtp_task_id');
        if (taskId) {
            window.taskId = taskId.value;
            window.reportProgress = document.getElementById('reportProgress');
            window.reportUrl = url.resolveScript({
                scriptId: 'customscript_cl_sl_task_util',
                deploymentId: 'customdeploy_cl_sl_task_util'
            });
            try {
                window.folderId = document.getElementById('custpage_folder_id').value;
                window.fileName = document.getElementById('search_file_name').value;
            } catch (e) {
                log.debug('e', e);
            }
            window.index = 0;
        ...
        if (window.taskId && window.reportProgress && window.reportUrl) {
            ...
            window.processTimer = window.setInterval(function () {
                ...
                if (status == "PROCESSING") {
                    ...
                    if (window.folderId) {
                        var urls = getFileUrl(window.folderId, window.fileName);
    ...
    */
    function getFileUrl(folderId, fileName) {
        var rt = new Object();
        rt["folder"] = "/app/common/media/mediaitemfolders.nl?folder=" + folderId;
        var filters = [];
        if (fileName) {
    ...
    */
    function addProgressFields(op) {
        ...
        progress.updateDisplayType({ displayType: 'inline' });
        progress.updateDisplaySize({ height: 25, width: 200 });
        progress.updateLayoutType({ layoutType: 'normal' });
        var html = gHitHtml;
        if (op.errMsg) {
            ...
            // 1-添加MR脚本的taskId
            form.addField({ id: 'custpage_rtp_task_id', type: 'text', label: "rtpTaskId" }).updateDisplayType({ displayType: 'hidden' }).defaultValue = op.taskId;
            // 2-添加folder的id
            form.addField({ id: 'custpage_folder_id', type: 'text', label: "pathId" }).updateDisplayType({ displayType: 'hidden' }).defaultValue = op.folderId;
            // 3-添加预计生成的文件名
            form.addField({ id: 'custpage_file_name', type: 'text', label: "pathId" }).updateDisplayType({ displayType: 'hidden' }).defaultValue = op.fileName;
        }
        return form;
    }
    ...
    function getScheduledScriptStatusPage(customdeployId) {
        try {
            ...
            scriptSearch.run().each(function (result) {
                ...
                var rec = record.load({ type: record.Type.SCRIPT_DEPLOYMENT, id: id });
                pageUrl = rec.getValue({ fieldId: 'instancestatuspage' });
                return false;
            });
            var scheme = 'https://';

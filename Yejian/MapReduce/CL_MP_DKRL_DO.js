 ...
 */
define(
    ['N/search', 'N/record', 'N/task','N/file','N/runtime','N/format','N/currency'],
    function(search, record, task,file,runtime,format,nsCurrency) {
        function getInputData() {
            var scriptObj = runtime.getCurrentScript();
            //到款通知id
            var recid = scriptObj.getParameter({name:'custscript_recid'});
            //到款通知类型
            var recordType = scriptObj.getParameter({name:'custscript_rectype'});
            //关联销售订单id
            var soId = scriptObj.getParameter({name:'custscript_soid'});
            //净美元
            var usd = scriptObj.getParameter({name:'custscript_usd'});

            //操作
            // dkrlDo({"recid":recid,"soId":soId,"usd":usd,"recordType":recordType});

            return {"recid":recid,"soId":soId,"usd":usd,"recordType":recordType};
        }
        ...
        function dkrlDo(allData){
            ...
            if(recid){
                ...
                try{
                    ...
                    var new_arr = doArr(dk_arr,recid);
                    if(new_arr.length > 0){
                        record.submitFields({
                            type:'salesorder',
                            id:soId,
                            values:{
                                custbody_source_arrival_notice:new_arr,
                            }
                        });
                    }
                }

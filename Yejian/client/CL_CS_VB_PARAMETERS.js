 ...
 */
 ...
 function(currentRecord,format,urls,record,mysearch,https,runtime) {
     ...
     function search(){
        ...
        var item = record.getValue('custpage_item_arr');//货品
         if(item){
             item = encodeURIComponent(item);
             url = changeURLArg(url,'item',item);
         }
         var jq = record.getValue('custpage_jq');//精确搜索
         if(jq){
             jq = encodeURIComponent(jq);
             url = changeURLArg(url,'jq',jq);
         }
         var sx = record.getValue('custpage_shuxing');//属性
         if(sx){
             sx = encodeURIComponent(sx);
             url = changeURLArg(url,'sx',sx);
         }
     ...
     function refresh(){
        //  var url = 'https://5784666.app.netsuite.com/app/site/hosting/scriptlet.nl?script=248&deploy=1';//正式
         var url = 'https://7373203.app.netsuite.com/app/site/hosting/scriptlet.nl?script=93&deploy=1';//测试
         window.location.href = url;
     }
     ...
     function refreshItem(){
         //  var url = 'https://5784666.app.netsuite.com/app/site/hosting/scriptlet.nl?script=248&deploy=1';//正式
         var url = 'https://7373203.app.netsuite.com/app/site/hosting/scriptlet.nl?script=183&deploy=1';//测试
         window.location.href = url;
     }
     /**
     ...
     function checkRole(role_id) {
        ...
        var flag = 'N';
        if(role_id){
           // var role_arr = new Array(1040,1041,1043,1042,1050,1038,1046);//BSQ_仓储专员”“BSQ_仓储主管”“BSQ_关务经理”“BSQ_报关员”“BSQ_物流专员”“BSQ_物流主管”“BSQ_物流经理

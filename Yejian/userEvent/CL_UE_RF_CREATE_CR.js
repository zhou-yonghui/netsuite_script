...
define(['N/record','N/search','N/format','N/runtime','N/currency'],
    function(record,search,format,runtime,nsCurrency) {
                            ...
                            if(so_data != 'null'){
                                ...
                                if(refund_currency == 2 && flag == 'Y_1'){
                                    ...
                                    if(flag == 'Y_2'){
                                        ...
                                        var ret = createCustomerRefund(so_data,refund_date,pay_type,refund_type,de_id,refund_amount,dk_id,so_id);
                                        if(ret != 'false'){
                                            var g_id = createGhRec(rec,refund_date,pay_type,dk_currency,dk_refund_amount,dk_jmy,rate,dk_id,dk_rec);
                                            if(g_id){
                                                rec.setValue('custrecord_customer_refund',ret);
        ...
        function createGhRec(recTk,refund_date,pay_type,dk_currency,dk_refund_amount,dk_jmy,rate,dk_id,dk_rec){
            try{
                ...
                rec.setValue('custrecord_hg_bank_transfer_amount_f',dk_refund_amount);//支付金额
                rec.setValue('custrecord_hg_bank_transfer_amount_t',(Number(dk_refund_amount)/Number(rate)).toFixed(2));//购进金额
                rec.setValue('custrecord_hg_bank_transfer_gh_rate',(Number(dk_jmy)/Number(dk_refund_amount)).toFixed(4));//购汇汇率
                ...
                if(dk_currency == 1){//仅购进币种与本位币不一致
                    rec.setValue('custrecord_hg_bank_transfer_type',3);//类型
                    rec.setValue('custrecord_hg_bank_transfer_amount_hs',(dk_jmy * jz_rate).toFixed(2));//换算本位币金额
                }
                else if(dk_currency != 1){//支付币种与购进币种都和本位币不一致
                    rec.setValue('custrecord_hg_bank_transfer_type',1);//类型
                    rec.setValue('custrecord_hg_bank_transfer_amount_hs',(dk_jmy * gj_cny_rate * zf_cny_rate).toFixed(2));//换算本位币金额
                }
                if(jz_rate != 'null'){
                ...
                if(zf_cny_rate != 'null'){
                    ...
                    rec.setValue('custrecord_hg_bank_transfer_amount_fz',(dk_refund_amount * zf_cny_rate).toFixed(2));//中转科目金额（支付）
                }
                if(gj_cny_rate != 'null'){
                    rec.setValue('custrecord_hg_bank_transfer_bz_rate',(gj_cny_rate).toFixed(4));//购进汇率（本位币)
                    rec.setValue('custrecord_hg_bank_transfer_amount_tz',(dk_jmy * gj_cny_rate).toFixed(2));//中转科目金额（购进）
                }
                if(jz_rate > rate){

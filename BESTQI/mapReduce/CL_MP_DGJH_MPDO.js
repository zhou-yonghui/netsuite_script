...
define(
    ...
    function(search, record, task,file,exchangeRate,format,runtime) {
        function getInputData() {
            ...
            //
            var dgSearchId = scriptObj.getParameter({name:'custscript_do_search'});//订柜计划搜索id
            if(dgSearchId == 'A'){
                button_flag = 1;
            }
            else if(dgSearchId == 'customsearch_dg_fh_pl_cl'){//发运
                button_flag = 2;
            }
            else if(dgSearchId == 'customsearch_sh_qc_cl'){//收货
                button_flag = 3;
            }
            else if(dgSearchId == 'D'){//发货
                button_flag = 4;
            }
            //加载搜索
            var mySearch = search.load(dgSearchId);
        ...
        function map(context) {
            ...
            if(value){
                doSomething(value.dgId,value.button_flag);
                // doSomething(1117,value.button_flag);
            }
        }
        function doSomething(dgId,button_flag) {
            ...
            if(dgId && button_flag){
                ...
                var document_status = dg_rec.getValue('custrecord_document_status');//单据状态，已发运：2,已完结 ：6
                ...
                if(document_status != 1){   //判断状态
                    ...
                    for(var i = 0;i < dg_count;i++){
                        ...
                        if(shipment_type_flag == 'outsea'){
                            ...
                            if(button_flag == 2 || button_flag == 4 || button_flag == 5){
                                ...
                                //发货，生成内部to单
                                    ...
                                    //创建内部to关联单据
                                    save_flag = createNbTo(ns_type,dg_rec,nb_to,to_arr,nb_to_id,shipment_item_type_t,actual_shipment_quantity,cumulative_shipment_quantity,start_location,end_location,sku,to_flag,save_flag);
                                    // //赋值第一次收货时间
                            ...
                            if(actual_qty > 0){
                                ...
                                if(shipment_type_flag == 'gnout'){
                                    if(button_flag == 2 || button_flag == 5){
                                        if(actual_qty > 0 && ns_type != 4 && one_nb_po.length == 0){
                                            ...
                                            //创建内部to关联单据
                                            save_flag = createNbTo(ns_type,dg_rec,nb_to,to_arr,nb_to_id,shipment_item_type_t,actual_qty,cumulative_shipment_quantity,start_location,end_location,sku,to_flag,save_flag);
                                            //赋值第一次收货时间
                                            // if(save_flag == 'Y' && !ir_one_date){
                                            //     dg_rec.setCurrentSublistValue({
                                            //         sublistId:'recmachcustrecord_sl_rp_body',
                                ...
                                //有po单，供应商直发FBA和供应商发海外
                                else if(shipment_type_flag == 'vendor' && po_id){
                                    ...
                                    if(wb_po_ir.length == 0 && wb_po_vb.length == 0){
                                        if(button_flag == 1 || (button_flag == 2 && is_wb_po == false)){
                                            //外部po收货
                                            var do_po_data = wbPoToiriv(po_id,actual_qty,wb_po_arr,sku);
                        ...
                        //收货，生成IT单
                        if(actual_receipt_quantity > 0){//完结、发运状态都可收货
                            if(button_flag == 3 || button_flag == 5){
                                save_flag = 'F';
                    ...
                    if(do_save == 'Y'){
                        ...
                        if(it_one_flag != 'Y'){
                            ...
                            //     dg_rec.setValue('custrecord_sl_in_out_check',false);//入库/出库回写
                            // }
                        }
                        //赋值签收时间，取第一次收货时间
                        ...
                        if(button_flag == 2){
                            ...
                            dg_rec.setValue('custrecord_document_status',2);//单据状态,已发运 2a
                        }
        ...
        function createNbTo(ns_type,dg_rec,nb_to,to_arr,nb_to_id,shipment_item_type_t,actual_shipment_quantity,cumulative_shipment_quantity,start_location,end_location,sku,to_flag,save_flag) {
            var ifir_flag = 'N';
            if(ns_type == 4){
                if(nb_to.length > 0){
                    to_arr = arrayAdd(nb_to,to_arr);
                    try{
                        nb_to_id = createTo(shipment_item_type_t,actual_shipment_quantity,start_location,end_location,sku);
                    }catch(e){
        ...
        function checkShipmentType(shipment_item_type_t) {
            ...
            if(shipment_item_type_t){
                if(shipment_item_type_t.indexOf("供应商直发海外仓") != -1 || shipment_item_type_t.indexOf("供应商直发FBA") != -1){
                    flag = 'vendor';
                }
                else if(shipment_item_type_t.indexOf("国内直发FBA") != -1 || shipment_item_type_t.indexOf("国内直发海外仓") != -1){
                    flag = 'gnout'
                }
                else if(shipment_item_type_t.indexOf("海外仓转平台仓") != -1 || shipment_item_type_t.indexOf("海外仓转海外仓") != -1 || shipment_item_type_t.indexOf("FBA转海外仓") != -1){
                    flag = 'outsea';
                }
            }
            return flag;
        }
        function wbPoToiriv(po_id,actual_qty,wb_po_arr,line_sku) {
            var do_flag = 'Y';
            var ir_Id;
            var vendorbill_Id;
            var ir_error = 'n';
                    ...
                    if(order_status == 'B' || order_status == 'D' || order_status == 'E'){//B:待收货；D:部分收货；E:部分收货/待定账单
                        try{
                            ...
                            for(var i = 0; i < ir_count;i++){
                                ...
                                // log.debug('sku line_sku',sku + '---' + line_sku);
                                if(sku == line_sku){
                                    var location = ir_Rec.getCurrentSublistValue({
                                        sublistId:'item',
                                        fieldId:'location',
                                    });
                                    ...
                                    // log.debug('sku location qty is_receive',sku + '---' + location + '---' + qty + '----' + is_receive);
                        ...
                        try{
                            ...
                            if(ir_Id){
                                ...
                                for(var j = 0;j < vb_count;j++){
                                    ...
                                    // log.debug('vb_sku line_sku',vb_sku + '---' + line_sku);
                                    if(vb_sku == line_sku){
                                ...
                                if(vb_sku == line_sku){
                                    if(vb_diff_qty2 == 0){
                                        if(vb_qty >= actual_qty){
                                            ...
                                            vendorbill_Rec.commitLine('item');
                                            vb_diff_qty2 = Number(actual_qty) - Number(vb_qty);
                                        }
                                    }
                                    else {
                                        if(vb_qty >= vb_diff_qty2){
                                            vendorbill_Rec.setCurrentSublistValue({
                                                sublistId:'item',
                                                fieldId:'quantity',
                                                value:vb_diff_qty,
                                            ...
                                            vendorbill_Rec.commitLine('item');
                                            vb_diff_qty2 = Number(vb_diff_qty2) - Number(vb_qty);
                                        }
                                    }
                                }
                            }
